...
def samplePdeActiveVar(numSamples, activeSamples, numKL, xi_k, mesh,
                       KL_obj, bcs, bcs_adj, Chi_1, Chi_2, read_in={}):

    # Get the eigenfunctions and eigenvalues
    if read_in:
        eigen_func = np.empty((numKL), dtype=object)
        eigen_val = np.empty((numKL), dtype=float)
        v_temp = np.empty((numKL), dtype=float)
        V = FunctionSpace(mesh, "CG", 1)
        for eigen_pairs in range(0, numKL):
            eigen_func[eigen_pairs] = Function(V)
            v_temp = \
                np.ascontiguousarray(read_in['KL_eigen_funcs'][eigen_pairs, :])
            eigen_func[eigen_pairs].vector()[:] = v_temp
        eigen_val = read_in['KL_eigen_vals'][0, :]
    else:
