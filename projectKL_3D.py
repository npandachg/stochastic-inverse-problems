...
class projectKL(object):
    ...
    def getCovMat(self, cov_expr):
        ...
        xyzcor = np.empty((6),dtype=float) # the points to evalute the expression
        ...
        for node_i in range(0,self.domain.getNodes()):
            ...
            for node_j in range(node_i,self.domain.getNodes()):
                ...
                for elem_i in self.node_to_elem[node_i]:
                    ...
                    # y1 : y co-ordinate of the centroid of element elem_i
                    y1 = self.c_centroid_array[elem_i].y()
                    # z1 : z co-ordinate of the centroid of element elem_i
                    z1 = self.c_centroid_array[elem_i].z()
                    for elem_j in self.node_to_elem[node_j]:
                        ...
                        # z2 : z co-ordinate for the centroid of element elem_j
                        z2 = self.c_centroid_array[elem_j].z()
                        xyzcor[0] = x1
                        xyzcor[1] = x2
                        xyzcor[2] = y1
                        xyzcor[3] = y2
                        ...
                        cov_expr.eval(cov_ij,xyzcor)
                        if cov_ij[0] > 0:
			    temp_cov_ij += (1./16)*cov_ij[0]*self.c_volume_array[elem_i]* \
